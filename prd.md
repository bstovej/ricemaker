# Project Requirements: Ricemaker (v2.2, 2026-07-10)
## 1. Executive Summary
...
4.  **Inference:** `LiteLLM` calls the configured model based on the selected `llm_provider` in `config.json` (e.g. using `llama_cpp` for llama.cpp OpenAI-compatible server or `ollama` for Ollama endpoint).
...
* **Resource Strategy:** Use the Host machine's GPU for llama.cpp or Ollama to maximize speed and handle large files, while keeping the application logic isolated in Docker.

### 4.2 Docker Configuration
* **Networking:** The container uses `host.docker.internal` to bridge to the local inference services running on the host OS.
...
### 4.3 Setup Steps
1.  **Host Config:** Install llama.cpp or Ollama on the Mac/Windows host.
2.  **Secrets:** Populate `keys.json` with API keys, `LLAMA_CPP_API_BASE`, and `OLLAMA_API_BASE`.
3.  **Config:** Set absolute paths for input/output and configure `llm_provider` as `llama_cpp` or `ollama` in `config.json`.
...
* **Privacy:** If the model is set to local backends (llama.cpp or Ollama), no document content should leave the local network.
